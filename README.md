...
이 프로그램의 `mac_nested()`는 실제 NPU를 사용하지 않고 Python으로 NPU의
MAC 연산을 흉내 낸 **CPU 단일 스레드 기준 구현**입니다. CPU에서는 이중
반복문이 행렬 원소를 하나씩 읽어 `score += pattern × filter`를 차례로
실행합니다. 앞 연산의 누적값이 다음 연산에 이어지므로 Python 반복문과
...
결합은 그대로 남습니다. 따라서 위 성능 표는 **CPU 직렬 구현의 N² 증가를
보여 주는 결과**이며, 실제 NPU와의 속도 차이를 측정한 결과로 해석하지
않았습니다.
...
18. CPU 단일 스레드 구현은 Python 반복문과 순차 누적이 병목이고, 실제 NPU는
    MAC을 병렬화한 뒤 메모리 대역폭·데이터 이동·부분합 결합이 병목이 됩니다.
19. 현재 성능 측정은 CPU에서 실행한 시뮬레이터의 결과이므로 실제 NPU 가속
    비율을 의미하지 않습니다.
