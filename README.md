...
현재 구현된 기능이 아니라 큰 N 지원을 추가할 때의 권장 순서입니다.
...
| 우선순위 | 기법 | 적용 방법과 이유 |
|---:|---|---|
| 1 | 스트리밍·출력 제한 | 패턴과 필터를 한 행씩 공급해 두 점수에 바로 누적하고 처리한 행은 버립니다. 입력과 필터까지 행 단위로 공급하면 작업 메모리를 O(N) 수준으로 제한할 수 있습니다. 기본 출력은 크기·점수·판정만 보여 주고 전체 행렬은 명시적으로 요청할 때만 출력합니다. |
| 2 | 연산 융합(단일 순회) | 한 좌표를 읽을 때 Cross와 X 점수를 함께 갱신하고 검증도 한 번만 수행합니다. 반복 읽기와 복사는 줄지만 필터가 두 개이므로 곱셈은 여전히 2N²회이고 시간 복잡도도 O(N²)입니다. |
| 3 | 타일링·블록화 | 행 단위 처리가 어렵거나 NPU의 로컬 메모리를 사용할 때는 B×B 블록만 올려 부분합을 누적합니다. 작업 블록은 O(B²)로 제한하고 블록 크기는 로컬 메모리 용량과 실제 측정 결과에 맞춰 정합니다. |
| 4 | 조건부 병렬화 | 프로파일링에서 계산이 병목이고 블록이 충분히 클 때만 독립 블록을 병렬 처리합니다. Python 스레드는 CPU 중심 반복문에 적합하지 않으므로 프로세스·네이티브 코드·NPU를 검토하되, 작은 입력에서는 분배·복사·부분합 결합 비용이 더 클 수 있습니다. |

현재의 전체 JSON 로딩과 리스트 기반 행렬 생성을 그대로 두고 MAC 반복문만
스트리밍이나 타일링으로 바꾸면 총 메모리 사용량은 O(N²)로 남습니다. 입력과
필터를 행 또는 블록 단위로 공급하도록 데이터 경로도 함께 바꿔야 합니다.
정확한 MAC을 위해 모든 좌표를 확인하므로 총 연산량 O(N²)은 바뀌지 않으며,
병렬 부분합은 덧셈 순서를 바꿀 수 있으므로 epsilon 경계 결과도 다시 검증해야
합니다. 따라서 메모리와 출력 병목을 먼저 줄이고, 측정에서 계산이 병목으로
남을 때만 병렬화를 적용합니다.
