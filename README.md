...
1. 필터 키 `cross`와 예상값 `+`는 `Cross`로, `x`는 `X`로 정규화했습니다.
2. `size_5_1`, `size_13_2`, `size_25_1`은 두 점수가 각각 `0.9`, `7.5`,
   `4.9`로 같아 `UNDECIDED`가 됐습니다. 예상 라벨은 차례로 `X`, `Cross`,
   `X`입니다.
3. 나머지 세 케이스의 점수 차이는 `8.8`, `14.4`, `52.8`이어서 예상 라벨과
   같은 결과가 나왔습니다.
4. 세 FAIL은 동점 정책을 그대로 적용한 예상 결과이며 프로그램 오류가 아닙니다.

### 비교와 오류 처리

5. 두 점수의 차이가 `1e-9`보다 작을 때만 `UNDECIDED`로 판정합니다.
6. 차이가 epsilon과 정확히 같으면 동점이 아닌 것으로 처리하고 이 경계도
   테스트로 고정했습니다.
7. 잘못된 크기나 라벨은 해당 케이스만 FAIL로 기록해 다음 분석을 이어갑니다.
8. FAIL 메시지는 데이터·스키마 오류와 수치 비교, 판정 불일치를 구분합니다.

### 성능 분석

9. N×N MAC의 곱셈 횟수는 9, 25, 169, 625처럼 N²으로 늘어납니다. 실제
   측정에서도 25×25가 3×3보다 오래 걸렸습니다.
10. 짧은 실행은 운영체제 스케줄링과 Python 인터프리터 오버헤드의 영향을 받아
    측정값이 N² 비율과 정확히 맞지 않습니다. 복잡도는 이중 반복문과 연산
    횟수에서 더 분명하게 확인됩니다.
11. 1D 비교는 평탄화를 측정 전에 끝내고 같은 입력과 반복 횟수로 진행했습니다.
    대체로 빨랐지만 25×25도 작은 입력이므로 항상 빠르다고 단정하지 않았습니다.
12. CPU 구현은 Python 반복문과 순차 누적이 병목입니다. NPU는 MAC을 병렬화한
    뒤 메모리 대역폭·데이터 이동·부분합 결합이 병목이 됩니다. 현재 측정값은
    CPU 시뮬레이터의 결과이며 실제 NPU 가속 비율을 뜻하지 않습니다.
